...
function pvwp_profile(array $profile): void {
    ...
    <div class="pvn-profile-info"><p class="pvn-eyebrow"><?php pvwp_label('profile.statusBanner'); ?></p><h1><?php echo esc_html($profile['title']); ?></h1><p class="pvn-profile-age"><?php pvwp_label('profile.ageYears', array('age' => $data['age'] ?? '')); ?></p><div class="pvn-tags"><?php foreach (($data['cities'] ?? array()) as $key) { $city = pvc_record('city', pvwp_context()['locale'], $key); if ($city) { ?><a href="<?php echo esc_url(pvwp_url($key)); ?>"><?php echo esc_html($city['title']); ?></a><?php } } ?></div><p class="pvn-availability" data-status="<?php echo esc_attr($data['availability'] ?? 'on-request'); ?>"><?php pvwp_label('profile.availability.' . ($data['availability'] ?? 'on-request')); ?></p>
